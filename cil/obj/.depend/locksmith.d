      $(OBJDIR)/locksmith.cmo: \
          $(OBJDIR)/uniqueness.cmi \
          $(OBJDIR)/shared.cmi \
          $(OBJDIR)/semiunification.cmi  $(OBJDIR)/rmtmps.cmi \
          $(OBJDIR)/rmalias.cmi \
          $(OBJDIR)/lockutil.cmi \
          $(OBJDIR)/locktype.cmi \
          $(OBJDIR)/lockstate.cmi \
          $(OBJDIR)/locksettings.cmi \
          $(OBJDIR)/lockprofile.cmi \
          $(OBJDIR)/labelflow.cmi  $(OBJDIR)/errormsg.cmi \
          $(OBJDIR)/dotpretty.cmi \
          $(OBJDIR)/correlation.cmi \
          $(OBJDIR)/controlflow.cmi  $(OBJDIR)/cil.cmi \
          $(OBJDIR)/bansheemlifc.cmi \
          $(OBJDIR)/locksmith.cmi 
      $(OBJDIR)/locksmith.cmx: \
          $(OBJDIR)/uniqueness.cmx \
          $(OBJDIR)/shared.cmx \
          $(OBJDIR)/semiunification.cmx  $(OBJDIR)/rmtmps.cmx \
          $(OBJDIR)/rmalias.cmx \
          $(OBJDIR)/lockutil.cmx \
          $(OBJDIR)/locktype.cmx \
          $(OBJDIR)/lockstate.cmx \
          $(OBJDIR)/locksettings.cmi \
          $(OBJDIR)/lockprofile.cmx \
          $(OBJDIR)/labelflow.cmx  $(OBJDIR)/errormsg.cmx \
          $(OBJDIR)/dotpretty.cmx \
          $(OBJDIR)/correlation.cmx \
          $(OBJDIR)/controlflow.cmx  $(OBJDIR)/cil.cmx \
          $(OBJDIR)/bansheemlifc.cmx \
          $(OBJDIR)/locksmith.cmi 
